...
const path = "/where-to-stay-lucca-inside-walls";
const pageCopy: Record<Locale, { kicker: string; note: string }> = {
  en: {
    kicker: "Lucca guide",
    note: "Note tra le Mura is at Via Pelleria 14, inside Lucca’s walls.",
  },
  it: {
    kicker: "Guida di Lucca",
    note: "Note tra le Mura si trova in Via Pelleria 14, dentro le mura di Lucca.",
  },
  de: {
    kicker: "Lucca-Guide",
    note: "Note tra le Mura liegt in der Via Pelleria 14, innerhalb der Stadtmauern von Lucca.",
  },
  sr: {
    kicker: "Vodič za Luku",
    note: "Note tra le Mura nalazi se u Via Pelleria 14, unutar zidina Luke.",
  },
};
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const guide = travelGuides["where-to-stay-lucca-inside-walls"][locale];
  return {
...
export default async function WhereToStayGuidePage({ params }: Props) {
  const { locale: raw } = await params;
  if (!isLocale(raw)) {
    notFound();
  }
