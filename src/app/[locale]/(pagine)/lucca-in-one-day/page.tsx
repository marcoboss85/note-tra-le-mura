...
const path = "/lucca-in-one-day";
const pageCopy: Record<Locale, { kicker: string; note: string }> = {
  en: {
    kicker: "Lucca guide",
    note: "Note tra le Mura is at Via Pelleria 14, inside Lucca’s walls — a practical base for a one-day visit.",
  },
  it: {
    kicker: "Guida di Lucca",
    note: "Note tra le Mura si trova in Via Pelleria 14, dentro le mura — base ideale per visitare Lucca in un giorno.",
  },
  de: {
    kicker: "Lucca-Guide",
    note: "Note tra le Mura liegt in der Via Pelleria 14, innerhalb der Mauern — ein guter Ausgangspunkt für einen Tagesbesuch.",
  },
  sr: {
    kicker: "Vodič za Luku",
    note: "Note tra le Mura je u Via Pelleria 14, unutar zidina — praktična baza za obilazak Luke za jedan dan.",
  },
};
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const guide = travelGuides["lucca-in-one-day"][locale];
  return {
...
export default async function LuccaInOneDayGuidePage({ params }: Props) {
  const { locale: raw } = await params;
  if (!isLocale(raw)) {
    notFound();
  }
